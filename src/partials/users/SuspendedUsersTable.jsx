...
import SuspendedUsersTableItem from "./SuspendedUsersTableItem";
  ...
  const fetchUsers = async (limit = rowsPerPage, currentPage = page) => {
    ...
    try {
      const response = await fetch(
        `${apiURL}/api/admin/users/suspended?limit=${limit}&page=${currentPage}`,
        {
          method: "GET",
          headers: {
          ...
          <div>
            ...
            <div className="overflow-x-auto rounded-2xl px-7">
              <table className="table-auto w-full dark:text-gray-300 divide-y divide-gray-100 dark:divide-gray-700/60 -11">
                ...
                {suspendedUsers?.data?.map((user) => {
                  return (
                    <SuspendedUsersTableItem
                      key={user.reference}
                      id={user.reference}
            ...
            {suspendedUsers?.data?.length > 0 && (
              <div>
                <Pagination
                  totalItems={suspendedUsers?.meta?.total || 0}
                  page={page}
