...
function ReportedContentsTableItem(props) {
  return (
        ...
        <td className="px-2 first:pl-0 last:pr-5 py-3.5 whitespace-nowrap">
          <div className="flex items-center text-gray-800">
            ...
            <div className="font-semibold text-[16px] text-black">
              <p> {props.user}</p>
              <p className="text-xs text-[#686868] font-medium">
                {props?.contentType?.map((item, idx) => (
                  <span key={item}>
                    {item}
                    {idx < props?.contentType?.length - 1 ? ", " : ""}
                  </span>
                ))}
              </p>
            </div>
...
export default ReportedContentsTableItem;
